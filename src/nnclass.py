...
def main():
    ...
    clf = MLPClassifier(random_state=42, # Setting random seed
                        hidden_layer_sizes=(100, 10), # Defining size of hidden layers
                        learning_rate="adaptive", # Adaptive = it will adjust during training
                        early_stopping=False, # Not stopping the training since there isnt much training data
                        verbose=True, 
                        max_iter=20).fit(X_train, y_train) # Sets the maximum iterations to 20 if it isnt stopped earlier
    ...
    file_name = "neural_network_classifier.txt"
